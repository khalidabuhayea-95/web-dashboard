...
import { ELEMENT_CATEGORY_SETTINGS } from "@/lib/elements/categorySettings";

let localCategorySeq = 0;

function createEmptyElementCategory() {
  return {
...
export default function ElementCategoriesSection({ canEdit }) {
  const [settings, setSettings] = useState(ELEMENT_CATEGORY_SETTINGS);
  const [expandedCategories, setExpandedCategories] = useState([]);
  const [loading, setLoading] = useState(true);
  const [busy, setBusy] = useState(false);
  const [status, setStatus] = useState("Loading element categories...");
  ...
  useEffect(() => {
    ...
    const loadSettings = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(payload?.error || "Failed to load element categories.");
        }

        if (!isMounted) return;
        setSettings(Array.isArray(payload?.settings) ? payload.settings : ELEMENT_CATEGORY_SETTINGS);
        setAssetCounts(payload?.counts && typeof payload.counts === "object" ? payload.counts : {});
        setStatus("");
      } catch (error) {
        if (!isMounted) return;
        setStatus(error?.message || "Failed to load element categories.");
      } finally {
  ...
  const addCategory = useCallback(() => {
    setSettings((current) => [...current, createEmptyElementCategory()]);
  }, []);

  const removeCategory = useCallback(
    (index) => {
      // Removing a category deletes the elements inside it once the page is saved, so the
      ...
      if (count > 0) {
        ...
        const confirmed = window.confirm(
          `Remove "${name}"?\n\n${count} element${count === 1 ? "" : "s"} filed under it will be ` +
  ...
  const handleSave = async () => {
    setBusy(true);
    setStatus("Saving element categories...");
    try {
      const response = await fetch("/api/settings/element-categories", {
        method: "PUT",
      ...
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) {
        throw new Error(payload?.error || "Failed to save element categories.");
      }
      ...
      setStatus(
        removedCount > 0
          ? `Element categories saved. Removed ${removedCount} categor${removedCount === 1 ? "y" : "ies"} and deleted ${deletedAssets} element${deletedAssets === 1 ? "" : "s"}.`
          : "Element categories saved."
      );
    } catch (error) {
      setStatus(error?.message || "Failed to save element categories.");
    } finally {
      setBusy(false);
    }
  };
  ...
  const handleThumbnailUpload = useCallback(
    async (index, files) => {
      ...
      try {
        ...
        setStatus("Thumbnail uploaded. Save element categories to persist the change.");
      } catch (error) {
    ...
    <Card className="overflow-hidden">
      <CardHeader className="flex flex-row items-start justify-between gap-3">
        <div>
          ...
          <CardSubtitle>
            Manage the categories used when importing Magnific elements on the importer page.
          </CardSubtitle>
        </div>
        <Button
          type="button"
        ...
        <CardContent className="space-y-4">
            ...
            <div className="relative p-5 sm:p-6">
              ...
              >
                {status ? (
                  ...
                  <span className="text-muted-foreground">Element category changes stay local until you save.</span>
                )}
              </div>
          ...
          {loading ? (
            ...
            <div className="rounded-xl border border-dashed border-border bg-muted/30 p-6 text-sm text-muted-foreground">
              No element categories available.
            </div>
          ) : (
                ...
                return (
                  <article
                    ...
                    onDragStart={(event) => {
                      ...
                      event.dataTransfer.setData("application/x-element-category-index", String(categoryIndex));
                    ...
                    onDrop={(event) => {
                      ...
                      event.preventDefault();
                      const payload =
                        event.dataTransfer.getData("application/x-element-category-index") ||
                  ...
                  >
                    <header className="flex flex-wrap items-start justify-between gap-3 border-b border-border pb-3">
                      <div className="flex min-w-0 flex-1 items-start gap-3">
                        {canEdit ? (
                          <span
                            ...
                            title="Drag to reorder element category"
                            aria-hidden="true"
                          >
                          ...
                          <div className="flex flex-wrap items-center gap-1.5">
                            ...
                            {assetCount > 0 ? (
                              <Badge variant="neutral">
                                {assetCount} element{assetCount === 1 ? "" : "s"}
                              </Badge>
                            ) : null}
                          </div>
                        </div>
                      ...
                      <div className="flex flex-wrap items-start justify-end gap-2">
                        {canEdit ? (
                          <>
                            <Button
                              ...
                              aria-label={`${isPublished ? "Unpublish" : "Publish"} element category ${categoryIndex + 1}`}
                            >
                              {isPublished ? (
                                <>
                            ...
                            <Button
                              ...
                              aria-label={`Remove element category ${categoryIndex + 1}`}
                            >
                        ...
                        <Button
                          ...
                          aria-controls={`element-category-panel-${categoryIndex}`}
                        >
                          {isExpanded ? (
                            <>
                              <ChevronUp className="h-4 w-4" aria-hidden="true" />
                    ...
                    {isExpanded ? (
                      <div
                        id={`element-category-panel-${categoryIndex}`}
                        className="mt-4 grid gap-3 md:grid-cols-2"
                      >
                        <div className="space-y-1.5">
                          <Label htmlFor={`element-category-en-${categoryIndex}`}>Category English value</Label>
                          <Input
                            id={`element-category-en-${categoryIndex}`}
                        ...
                        <div className="space-y-1.5">
                          <Label htmlFor={`element-category-ar-${categoryIndex}`}>Category Arabic value</Label>
                          <Input
                            id={`element-category-ar-${categoryIndex}`}
                        ...
                        <div className="space-y-1.5 md:col-span-2">
                          <Label htmlFor={`element-category-thumbnail-${categoryIndex}`}>Category thumbnail URL</Label>
                          <Input
                            id={`element-category-thumbnail-${categoryIndex}`}
                        ...
                        <div className="space-y-1.5 md:col-span-2">
                          <Label htmlFor={`element-category-terms-${categoryIndex}`}>Magnific search terms</Label>
                          <Input
                            id={`element-category-terms-${categoryIndex}`}
                            value={formatSearchTerms(category?.searchTerms)}
                            onChange={(event) => updateCategory(categoryIndex, "searchTerms", event.target.value)}
                            placeholder="satin bow isolated png, ribbon banner sticker isolated"
                            disabled={!canEdit}
                          />
                          <p className="field-help text-muted-foreground">
                        ...
                        <div className="space-y-1.5">
                          <Label htmlFor={`element-category-source-${categoryIndex}`}>Import source</Label>
                          <Select
                            id={`element-category-source-${categoryIndex}`}
                            value={category?.importSource || "stock"}
                            onChange={(event) => updateCategory(categoryIndex, "importSource", event.target.value)}
                            disabled={!canEdit}
                          >
                            <option value="stock">Stock (v1/resources)</option>
                            <option value="icons">Icons (v1/icons)</option>
                          </Select>
                          <p className="field-help text-muted-foreground">
                            Icons guarantee transparency; stock cut-outs rely on the search term saying
                            &quot;isolated&quot; or &quot;png&quot;.
                          </p>
                        </div>

                        <div className="space-y-1.5">
                          <Label htmlFor={`element-category-content-type-${categoryIndex}`}>Import content type</Label>
                          <Select
                            id={`element-category-content-type-${categoryIndex}`}
                          ...
                          >
                            ...
                            <option value="psd">PSD (cut-out stickers)</option>
