...
// rendered art stay visually consistent in the app and the catalog PDF.
...
// Replaces one side of a template's before/after pair. The upload and the row
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (session.role !== "admin") {
      return handleForbidden("Only admins can replace AI template art");
    }
    ...
    let body: Buffer;
    try {
      body = await sharp(Buffer.from(await fileEntry.arrayBuffer()))
        .rotate()
    ...
    const uploaded = await uploadObject({
      bucket: BUCKET,
      key: `ai-templates/${template.slug}-${kind}-${uuid()}.jpg`,
      body,
    ...
    const thumbUrl = kind === "after" ? await publishCardThumb(body, "ai-templates", template.slug) : null;
    ...
    return handleApiError(error, "Failed to replace AI template art.");
  }
}
