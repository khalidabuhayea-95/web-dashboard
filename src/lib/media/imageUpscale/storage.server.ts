...
const logger = createLogger("media.image-upscale.storage");
...
function makeObjectPath(jobId: string, fileName: string, mimeType: string) {
  ...
  return `image-upscale/jobs/${year}/${month}/${day}/${safeJobId}/image-${safeBase}.${extension}`;
}
...
async function uploadBytesToBucket(bucket: string, objectPath: string, bytes: Buffer, mimeType: string) {
  try {
    ...
    throw createProcessingFailedError(
      error instanceof Error ? error.message : "Failed to upload image upscale asset."
    );
  }
}

export async function uploadImageUpscaleInputAsset(input: UploadStoredAssetInput) {
...
export async function createImageUpscaleSignedInputUrl(
  asset: StoredObject,
...
export async function deleteImageUpscaleStoredObjects(items: StoredObject[] = []) {
  ...
  await Promise.all(
    Array.from(grouped.entries()).map(async ([bucket, paths]) => {
      try {
        ...
        logger.warn("Failed to delete image upscale temp assets", {
          bucket,
          paths,
