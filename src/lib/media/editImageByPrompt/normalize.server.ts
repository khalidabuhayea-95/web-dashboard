...
const DEFAULT_MAX_INPUT_EDGE = 1536;
...
export type NormalizedEditImageInput = {
  image: NormalizedEditImageAsset;
  prompt: string;
  outputFileName: string;
};

function resolveMaxInputEdge(): number {
  const raw = Number.parseInt(String(process.env.IMAGE_EDIT_MAX_INPUT_EDGE || "").trim(), 10);
  if (Number.isSafeInteger(raw) && raw >= 256 && raw <= 4096) {
    return raw;
  }
  return DEFAULT_MAX_INPUT_EDGE;
}
...
async function decodeToCanvas({
  ...
  if (!ALLOWED_IMAGE_MIME_TYPES.has(detectedMimeType)) {
    throw createUnsupportedImageTypeError("Only PNG and JPEG images are supported for editing.");
  }
...
async function renderNormalizedAsset(
  ...
  width: number,
  height: number
): Promise<NormalizedEditImageAsset> {
  const outputCanvas =
...
export function normalizeEditPrompt(value: unknown): string {
  const raw = String(value ?? "").trim();
  if (!raw) {
    throw createInvalidPromptError("A text prompt describing the edit is required.");
  }
  if (raw.length > MAX_PROMPT_LENGTH) {
    throw createInvalidPromptError(`Prompt must be ${MAX_PROMPT_LENGTH} characters or fewer.`);
  }
  return raw;
}

export async function normalizeEditImageInput({
  imageBytes,
  imageMimeType,
  imageFileName,
  prompt,
  ...
  prompt?: unknown;
}): Promise<NormalizedEditImageInput> {
  const safePrompt = normalizeEditPrompt(prompt);
  ...
  const maxInputEdge = resolveMaxInputEdge();
  const fitted = fitInside(decodedImage.width, decodedImage.height, maxInputEdge, maxInputEdge, {
    allowUpscale: false,
  });
  ...
  return {
    ...
    prompt: safePrompt,
    outputFileName: `${fileBaseName(decodedImage.fileName, "image")}-edited.png`,
  };
}
